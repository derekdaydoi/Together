...
create index daily_states_user_id_idx on public.daily_states(user_id);
...
-- Security helpers. They are not exposed through the Data API.
...
create or replace function private.shares_couple(a uuid, b uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select a = b or exists (
    select 1
    from public.couple_members ma
    join public.couple_members mb on mb.couple_id = ma.couple_id
    where ma.user_id = a and mb.user_id = b
  );
$$;

create or replace function private.both_checked_in(p_couple_id uuid, p_week_start date)
returns boolean
language sql
stable
security definer
...
as $$
  ...
  where couple_id = p_couple_id and week_start = p_week_start;
$$;
...
-- Automatically add the creator as the owner. This trigger function is not callable through the API.
create or replace function private.add_couple_owner()
returns trigger
language plpgsql
security definer
set search_path = public
...
create trigger couples_add_owner
after insert on public.couples
for each row execute function private.add_couple_owner();

-- Hard limit: exactly the product constraint for V1 — a couple has at most two members.
...
begin
  -- Serialize membership changes for a couple so concurrent invite joins cannot exceed two members.
  ...
  if exists (select 1 from public.couple_members where user_id = new.user_id) then
    raise exception 'This user already belongs to a couple';
  end if;
  return new;
end;
$$;
...
-- RLS on every table exposed through the Data API.
...
using (private.shares_couple((select auth.uid()), id));
...
-- Joining is intentionally not open through Data API; the authenticated Edge Function handles invite-code joining.

create policy "daily_select_member" on public.daily_states
for select to authenticated using (private.is_couple_member(couple_id));
create policy "daily_insert_self" on public.daily_states
for insert to authenticated with check (private.is_couple_member(couple_id) and (select auth.uid()) = user_id);
create policy "daily_update_self" on public.daily_states
for update to authenticated using ((select auth.uid()) = user_id and private.is_couple_member(couple_id))
with check ((select auth.uid()) = user_id and private.is_couple_member(couple_id));
create policy "daily_delete_self" on public.daily_states
for delete to authenticated using ((select auth.uid()) = user_id and private.is_couple_member(couple_id));

create policy "work_select_member" on public.work_schedules
for select to authenticated using (private.is_couple_member(couple_id));
create policy "work_insert_self" on public.work_schedules
for insert to authenticated with check (private.is_couple_member(couple_id) and (select auth.uid()) = user_id);
create policy "work_update_self" on public.work_schedules
for update to authenticated using ((select auth.uid()) = user_id and private.is_couple_member(couple_id))
with check ((select auth.uid()) = user_id and private.is_couple_member(couple_id));
create policy "work_delete_self" on public.work_schedules
for delete to authenticated using ((select auth.uid()) = user_id and private.is_couple_member(couple_id));
...
for insert to authenticated with check (private.is_couple_member(couple_id) and (select auth.uid()) = created_by);
...
for delete to authenticated using ((select auth.uid()) = created_by and private.is_couple_member(couple_id));

create policy "checkins_select_private_then_reveal" on public.weekly_checkins
for select to authenticated
using (
  (select auth.uid()) = user_id
  or (private.is_couple_member(couple_id) and private.both_checked_in(couple_id, week_start))
);
create policy "checkins_insert_self" on public.weekly_checkins
for insert to authenticated with check (private.is_couple_member(couple_id) and (select auth.uid()) = user_id);
...
-- Start from deny-by-default for browser roles, then grant only operations used by the product.
revoke all on public.profiles, public.couples, public.couple_members, public.daily_states, public.work_schedules, public.availability_blocks, public.plans, public.weekly_checkins from anon, authenticated;
grant select, insert, update on public.profiles to authenticated;
grant select, insert, update (name) on public.couples to authenticated;
...
grant select, insert, update (title, starts_at, ends_at, plan_type, status, location, note, updated_at), delete on public.plans to authenticated;
grant select, insert, update on public.weekly_checkins to authenticated;

-- Private avatar bucket: image-only, max 2 MB.
...
using (
  ...
  and private.shares_couple((select auth.uid()), owner_id::uuid)
);
...
-- Realtime for the shared state tables.
alter publication supabase_realtime add table public.daily_states;
